"""Repository des événements de match (cache-first)."""
...
class EventsRepository(BaseRepository):
    name: str = "repo_events"
    provides: tuple[str, ...] = ("events",)
    volatility: str = "semi_rapide"

    def __init__(self, **kwargs: object) -> None:
        super().__init__(domain="events", **kwargs)  # type: ignore[arg-type]

    async def _fetch(self, key: str) -> Any:  # noqa: ANN401
        if self._client is None:
            return None
        params = _parse_key(key)
        raw = await self._client.fetch("/fixtures/events", params)
